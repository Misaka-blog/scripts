...
install_base(){
    ${PACKAGE_INSTALL[int]} curl bash wget sudo
}
...
install_x-ui(){
    ...
    if [ $# == 0 ]; then
        ...
        wget --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/sing-web/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
